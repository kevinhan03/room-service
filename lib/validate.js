...
function validateCards(cards) {
  ...
  return cards.map((card, index) => {
    ...
    const textStyle = normalizeTextStyle(card?.textStyle || card?.text_style);
    const mediaType = card?.mediaType === "video" || card?.media_type === "video" ? "video" : "photo";
    return { title, copy, imageUrl, textStyle, mediaType };
  });
}

const textStylePositions = new Set(["top", "center", "bottom"]);
const textStyleAligns = new Set(["left", "center", "right"]);

function normalizeTextStyle(style) {
  if (!style || typeof style !== "object") return {};
  const result = {};
  if (textStylePositions.has(style.position)) result.position = style.position;
  if (textStyleAligns.has(style.align)) result.align = style.align;
  const fontScale = Number(style.fontScale);
  if (Number.isFinite(fontScale)) result.fontScale = Math.min(1.8, Math.max(0.6, fontScale));
  if (typeof style.color === "string" && /^#[0-9a-f]{6}$/i.test(style.color)) result.color = style.color.toLowerCase();
  return result;
}
