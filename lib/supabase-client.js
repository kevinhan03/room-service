...
async function supabaseRequest(pathname, options = {}) {
  ...
  if (!response.ok) {
    const migrationMissing = [400, 404].includes(response.status)
      && ["human_decision", "human_saved", "last_recommended_at", "recommendation_count", "image_url", "kevin_inbox_items", "kevin_inbox_ideas", "why_i_like_this", "kevin_angle", "personal_relevance_score", "why_note_updated_at", "recommendation_events", "personal_score", "score_breakdown", "matched_preferences"].some((name) => text.includes(name));
    if (migrationMissing) {
