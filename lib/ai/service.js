...
const { cleanString, normalizeScore, validateReferenceUrls, validateResearchGenerated, slugFromUrl } = require("../validate");
...
async function callOpenAIInboxIdeas(item, research) {
      ...
      input: [
        {
          ...
          content: [
            ...
            "The goal is not to summarize news. Turn Kevin's clue into eight specific subjects he could genuinely want to post.",
            ...
            "The eight ideas must be meaningfully different, not alternate headlines for the same subject.",
            "Score visual_score, research_score, and novelty_score from 0 to 100.",
  ...
  if (ideas.length !== 8) {
    throw new AppError("Model returned fewer than eight ideas.", 502, "MODEL_OUTPUT_INVALID");
  }
  return ideas.map((idea, index) => ({
    ...
    visual_score: normalizeScore(idea.visual_score),
    research_score: normalizeScore(idea.research_score),
    novelty_score: normalizeScore(idea.novelty_score)
  }));
}
